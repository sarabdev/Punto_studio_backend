export const generateStructuredAnalysis = (text: string) => {
  return `
...
Note:First of all go throught the document check what the document is about like if the text is resume the what your main purpuse list all the positions,skill,educations,skill detail in your own word,if the
...
If the document includes examples, present these with a detailed, step-by-step walkthrough of the solution.
If examples are provided without solutions, create and detail the step-by-step process to solve these examples.

I want the result in markdown so please provide me a markdown and extract the import key topic from the document if it has and give a detail explanation of that sub topic via through document provided or if
document provided do not have then please add it.

As an example when the provide document is:    DIRECTION FIELDS
...
PLease provide detailed subtopic in list and also if the provide document has any questions or examples or any exercise questions please solve that examples, questions and exercise step-by-step.Also provide the code if nessosory and code structure.
i am showing you some dummy picture:

## Key Concepts or Terminology    // make Suitable heading name here
Identify and define fundamental concepts crucial to understanding the document. Elaborate on their relevance and implications concerning the subject matter.Make Key concept as long as you can
1. Title:
     Description
...
- For each important term or concept identified in the document, provide a detailed explanation, perhaps with bullet points for clarity.
...
Note: Include this section only if there are explicit questions within the document.
1. **Question:** Extracted Question
   ...
   (also extract the question in the provide document and provide step by step solution)

## Examples                     // if nessosory
Note: Include this section only if examples are found within the document.
...
-Outline crucial developments or arguments present in the document, offering a concise yet thorough analysis.If there are not then please not add this heading.

### Detailed Analysis              // if nessosory
-Provide a deeper examination of specific topics or claims from the document, supported by factual evidence, theories, or interpretations.If there are not then please not add this heading.

## Data, Statistics, and Figures         // if nessosory
-Detail any significant data, statistics, or figures mentioned in the document, illustrating their importance within the context of the subject.If there are not then please not add this heading.
...
-Compile common queries or specific questions naturally arising from the document, offering clear and concise responses.If there are not then please not ass this heading.

## Core Content             // if nessosory
_In-Depth Analysis and Discussion of Main Subject Matter_

## Comparative Analysis            // if nessosory
-Compare various viewpoints, models, or approaches within the document, discussing their contrasts and respective merits.

## Implications or Conclusions               // if nessosory
-Summarize potential implications derived from the information or draw conclusions arising from the analysis. Highlight any calls to action or future directions suggested by the document.

This is just a dummy picture you have to make accordingly the provided document context because everytime it not contain the same thing .thanks!!
Please provide the detailed of every important topic in the provided text make a topic details in a list and also if has any question then step by step solution.
and please do not add any discription of what you do like at the end of generating you day this document is bla bla dont add that just add the details more and more from the provided document if provided document has any important word or key point or event with less discription please and more detail in it.Thanks!
At the end please dont add this type of document:
Please Note: The provided document does not include specific questions, calculations, or code examples. Therefore, no step-by-step solutions, code structures, or technical details are included in the above output.If the something is not then dont say this is not so donot do......If you think that topic is nessosory then add it from your side with details.Dont make the heading of markdown just give me the markdow!!
Please dont provide any note at the end!you are also making the markdown as heading on the start then giving markdown please dont add markdown as heading!!!
Note:Please check the language of the provide document and and also reply in language in which the document is!
make sure reply me in the language of provided document.

Provided Document is this following:
------
